...
def solve_p1(data, *args, **kwargs):
    data = [[x if x not in "abcdefghijklmnopqrtstuvwxyz\n" else None for x in y] for y in data]
    data = [[list(filter(lambda x: x is not None, x))[0], list(filter(lambda x: x is not None, x))[-1]] for x in data]
