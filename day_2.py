...
def solve_p1(data, *_):
    ...
    for datapoint in data:
        datapoint = re.sub(r"Game \d+: ", "", datapoint)
        ...
        datapoint = [
            [
                [z if z not in "abcdefghijklmnopqrtstuvwxyz" else None for z in y]
                for y in x
            ]
            for x in datapoint
        ]
        datapoint = [
            [
                int("".join(list(filter(lambda x: x is not None, y))).replace(" ", ""))
                for y in x
            ]
            for x in datapoint
        ]
